...
export default function GradesFilter({ expedicaoData }: GradeFilterProps) {
  ...
  const statusBorders: any = {
    EXPEDIDA: 'border-green-300',
    DESPACHADA: 'border-blue-300',
    PRONTA: 'border-gray-300',
    IMPRESSA: 'border-purple-300',
    TODAS: 'border-pink-300',
  };

  // Fundo com transparência mínima e bordas mais suaves
  const statusBackgrounds: any = {
    EXPEDIDA: 'bg-green-600 bg-opacity-5',  
    DESPACHADA: 'bg-blue-600 bg-opacity-5', 
    PRONTA: 'bg-gray-300 bg-opacity-5', 
    IMPRESSA: 'bg-purple-600 bg-opacity-5', 
    TODAS: 'bg-gray-800 bg-opacity-5', 
  };
    ...
    <div className="w-full px-6 py-4 bg-[#181818]">
      {Object.entries(groupedByStatus).map(([status, grades]) => {
        ...
        return (
          ...
          >
            <h2 className="text-2xl font-bold text-zinc-300 mb-4 uppercase">{`STATUS: ${status}`}</h2>
            {grades.map((grade) => {
              ...
              return (
                ...
                >
                  <h3 className="text-xl font-semibold text-zinc-300 uppercase">
                    {`PROJETO: ${grade.projectname} - ${grade.company}`}
                  </h3>
                  <p className="text-zinc-500 uppercase">Escola: {grade.escola} (Nº {grade.numeroEscola})</p>
                  <p className="text-zinc-500 uppercase">Número Join: {grade.numberJoin}</p>
                  <div className="mt-4">
                    <h4 className="text-md font-semibold text-zinc-300 mb-2 uppercase">Itens Expedidos:</h4>
                    <table className="w-full text-zinc-500 border-collapse">
                      <thead>
                        <tr className={`border-b ${statusBorders[status]} border-opacity-30`}>
                          <th className="py-2 px-4 text-left uppercase">Item</th>
                          <th className="py-2 px-4 text-left uppercase">Gênero</th>
                          <th className="py-2 px-4 text-left uppercase">Tamanho</th>
                          <th className="py-2 px-4 text-left uppercase">Quantidade</th>
                        </tr>
                      </thead>
                      <tbody>
                  ...
                  <p className="text-zinc-500 mt-2 uppercase">
                    Total de itens da grade: <span className="font-bold">{totalQuantidade}</span>
                  </p>
                  <p className="text-zinc-500 uppercase">
                    Número de caixas: <span className="font-bold">{grade.caixas.length}</span>
                  </p>
                </div>
              );
            })}
            <div className="text-right text-zinc-500 font-semibold uppercase">
      ...
      <div className="text-center text-zinc-500 text-3xl font-bold mt-10 uppercase">
