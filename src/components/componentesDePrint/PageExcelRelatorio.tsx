...
export default function PageExcelRelatorio({ expedicaoData }: PageExcelRelatorioProps) {
    const generateExcel = async () => {
        ...
        const TOTiTENSeXPED = worksheet.addRow([
            "",
            "TOTAL GERAL DE ITENS EXPEDIDOS:",
            expedicaoData.reduce((sum, grade) => sum + grade.tamanhosQuantidades.reduce((s, i) => s + i.quantidade, 0), 0),
            "",
            ""
        ]);
        ...
        expedicaoData.forEach((grade) => {
            ...
            const totalRow = worksheet.addRow([
                "",
                "TOTAL DE ITENS PARA A ESCOLA:",
                grade.tamanhosQuantidades.reduce((sum, item) => sum + item.quantidade, 0),
                "",
                ""
            ]);
