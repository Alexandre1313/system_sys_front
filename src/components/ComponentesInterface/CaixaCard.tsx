...
const CaixaCard: React.FC<CaixaCardProps> = ({ caixa, tema }) => {
  ...
  const bgHeader = tema ? 'bg-zinc-200 text-zinc-800' : 'bg-[#1f1f1f] text-zinc-300';
  ...
  return (
    <div className={`w-full mb-8 border ${borderColor} rounded-md shadow-md`}>
      {/* Cabeçalho da caixa */}
      <table className={`w-full border-collapse border ${borderColor} table-fixed text-sm`}>
        <thead>
          <tr className={`${bgHeader} text-[16px]`}>
            <th className={`px-4 py-2 text-left border-r ${borderColor} w-[20%]`}>Caixa Nº</th>
            <th className={`px-4 py-2 text-left border-r ${borderColor} w-[50%]`}>Projeto</th>
            <th className={`px-4 py-2 text-left border-r ${borderColor} w-[20%]`}>Total Itens</th>
        ...
        <tbody>
          <tr
            ...
            onClick={toggleOpen}
          >
            <td className={`px-4 py-2 border-t border-r ${borderColor} font-semibold`}>
              {caixa.caixaNumber}
            </td>
        ...
        <table className={`w-full border-collapse border-t ${borderColor} table-fixed text-sm`}>
          <thead>
            <tr className={`${bgAlt} text-[15px]`}>
              ...
              <th className={`px-4 py-2 text-left border-r ${borderColor} w-[20%]`}>Tamanho</th>
              <th className={`px-4 py-2 text-left w-[20%]`}>Quantidade</th>
            </tr>
          </thead>
          <tbody>
            {caixa.caixaItem.map((item: any, index: number) => (
              <tr
                key={`${item.id}-${index}`}
