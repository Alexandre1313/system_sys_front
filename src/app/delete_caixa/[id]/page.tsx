...
import { getCaixaParaAjuste, modificarCaixaOuExcluir } from '@/hooks_api/api';
...
const fechBoxAjustOrExclud = async (box: CaixaAjuste): Promise<CaixaAjuste | null> => {
  const boxData = await modificarCaixaOuExcluir(box);
  return boxData;
}

export default function DeleteCaixa() {
  const { id } = useParams();
  ...
  const [msg, setMsg] = useState<string>(`DESEJA MESMO ALTERAR AS QUANTIDADES DA CAIXA DE ID ${id} OU EXCLUÍ-LA?`);
  ...
  const handleChange = (index: number, rawValue: number, status: string) => {
    ...
    const safeValue = Math.max(0, value); // Agora pode ser maior que originalQty
  ...
  const handleInputBlur = (index: number) => {
    ...
    if (currentValue === undefined || currentValue === null) {
      handleChange(index, 0, caixa!.status); // Se apagou o campo, vira 0
    }
  };
  ...
  const handleSaveStepTwo = async () => {
    ...
    try {
      ...
      const newBox = await fechBoxAjustOrExclud(novaCaixa);
      ...
      if (newBox) {
        ...
        if (refreshedBox) {
          ...
          setMsg('CAIXA ALTERADA / EXCLUIDA COM SUCESSO!');
          setMsg1('CONFIRA AS QUANTIDADES OU SE A CAIXA FOI EXCLUÍDA');
        } else {
          setMsg('ERRO AO ATUALIZAR OU EXCLUIR A CAIXA!');
          setMsg1('NÃO FOI POSSÍVEL CARREGAR A CAIXA ATUALIZADA OU EXCLUÍDA');
        }
      }
    } catch (error) {
      console.error('Erro ao salvar / excluir a caixa:', error);
      setMsg('OCORREU UM ERRO DURANTE A ATUALIZAÇÃO / EXCLUSÃO!');
      setMsg1('TENTE NOVAMENTE MAIS TARDE');
    } finally {
      const timeOut = setTimeout(() => {
        setMsg(`DESEJA MESMO ALTERAR AS QUANTIDADES DA CAIXA DE ID ${id} OU EXCLUI-LA?`);
        setMsg1('A OPERAÇÃO NÃO PODERÁ SER REVERTIDA');
        setOpenModal(!openModal);
    ...
    <>
      {isLoading ? (
        ...
        <div className="flex flex-col min-h-screen bg-[#181818] p-4 pb-20">
          {/* Cabeçalho fixo */}
          <TitleComponentFixed
            stringOne="AJUSTANDO / EXCLUINDO CAIXA"
            twoPoints=":"
            stringTwo={`${id}`}
          />
          ...
          <div className="fixed bottom-5 left-1/2 transform -translate-x-1/2 z-50">
            <button
              ...
              className={`bg-orange-900 text-white px-6 py-3 rounded-lg hover:bg-orange-800 shadow-xl uppercase ${cursor}`}
            >
              Salvar alterações
            </button>
          </div>
